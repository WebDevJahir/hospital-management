                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <div class="input-group">
                                        <input type="text" class="form-control" placeholder="Enter Doctor Name" id="doc_name" name="doc_name" minlength="2" required>
                                    </div>
                                    <!-- /.input group -->
                                </div>
                                <!-- /.form group -->
        ...
        <div class="row gutters">
            <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                ...
                <div class="table-container">
                    <div class="t-header">Doctor List @if(in_array(245, $permission)) <button type="button" class="btn-info btn-rounded" data-toggle="modal" data-target="#addModal">Add new Doctor</button> @endif </div>
                            ...
                            <tbody id="MedicalProcedureTable">
                                ...
                                <tr id="tr{{$doctor->id}}">
                                    ...
                                    <td>
                                        ...
                                        <button class="btn btn-sm" style="background:inherit" onclick="editDoctorView({{$doctor->id}})" title="Edit" type="submit"><i class="fas fa-edit text-success"></i></button>|
                                        @endif
                                        @if(in_array(247, $permission))
                                        <button class="btn btn-sm" style="background:inherit" onclick="deleteDoctorview({{$doctor->id}})" title="Delete" type="submit"><i class="fas fa-trash-alt text-danger"></i></button>
                                        @endif
                                    </td>
                                </tr>
                                @endforeach
...
$.post('{{url('edit-prescription-doctor')}}', {_token : '{{ @csrf_token() }}', id : id}, function(data){
...
function deleteDoctorview(id){
        ...
        if (result.isConfirmed) {
            $.post('{{url('delete-prescription-doctor')}}', {_token : '{{ @csrf_token() }}', id : id}, function(data){
                $('#tr'.concat(id)).remove();
                Swal.fire(
                            'Deleted!',
