                            ...
                            <div class="table-responsive">
                                <table id="Example" class="table custom-table">
                                    ...
                                    <tbody id="incomeHeadTable">
                                        @foreach ($medical_supports as $medical_support)
                                            <tr>
                                                <td>{{ $medical_support?->city?->name }}</td>
                                                <td>{{ $medical_support?->project?->name }}</td>
                                                <td>{{ $medical_support?->incomeHead?->name }}</td>
                                                <td>{{ $medical_support?->incomeSubCategory?->name }}</td>
                                                <td>{{ $medical_support->charge }}</td>
                                                <td>
                                                    {{-- <img src="{{ asset('uploads/medical-support/' . $medical_support->image) }}"
                                                    ...
                                                    <div class="icon-btn">
                                                        <nobr>
                                                            <a data-toggle="tooltip" title="Edit"
                                                                onclick="edit({{ $medical_support->id }})"
                                                                class="btn btn-outline-warning btn-sm"><i
                                                                    class="fas fa-pen"></i></a>

                                                            <form
                                                                action="{{ route('medical-support.destroy', $medical_support->id) }}"
...
@section('script')
    <script type="text/javascript">
        function edit(id) {
            let medical_supports = @json($medical_supports);
            medical_supports.find(medical_support => {
                if (medical_support.id == id) {
                    $.ajax({
                        url: "{{ route('get-income-heads-sub-category-edit') }}",
                        type: "GET",
                        data: {
                            project_id: medical_support.project_id,
                            income_head_id: medical_support.income_head_id
                        },
                        success: function(response) {
                            ...
                            $('select[name="city_id"]').val(medical_support.city_id);
                            $('select[name="project_id"]').val(medical_support.project_id);
                            $('select[name="income_head_id"]').val(medical_support.income_head_id);
                            $('select[name="income_sub_category_id"]').val(medical_support
                                .income_sub_category_id);
                            $('input[name="charge"]').val(medical_support.charge);
                            $('input[name="_method"]').val('PUT');
                            $('form').data('id', medical_support.id);
                            let form_url = "{{ route('medical-support.update', ':id') }}";
